...
void RobotArmTrajectoryGenerator::calculateCoefficient() {
    ...
    mFunctionValue.setOnes();
    mFunctionValue = 5 * mFunctionValue;
...
std::vector<double> RobotArmTrajectoryGenerator::getPositionTrajectory(double currentTime) {
    ...
    for (int i = 0; i < 6; ++i) {
        position[i] = mCoefficient(0,i) * pow(normalizedTime, 3.0) + mCoefficient(1,i) * pow(normalizedTime, 2.0) + mCoefficient(2,i) * normalizedTime + mCoefficient(3, i);
        std::cout << "position i" << i << " : " << position[i] << std::endl;
    }
    return position;
}
...
std::vector<double> RobotArmTrajectoryGenerator::getVelocityTrajectory(double currentTime) {
    ...
    for (int i = 0; i < 6; ++i) {
        velocity[i] = (3.0 *mCoefficient(0,i) * pow(normalizedTime, 2.0) + 2.0 * mCoefficient(1,i) * normalizedTime + mCoefficient(2,i)) / mTimeDuration;
    }
    return velocity;
}
