...
A1CollisionDetecter RearRightExternalTorqueObserver;
...
void raisimSimulation()
{
    ...
    if ((MainUI->button1) && (oneCycleSimTime < simulationDuration))
    {
        ...
        world.integrate();
        iteration++;
        RearRightExternalTorqueObserver.UpdateState(robot.getQ(),robot.getQD(),MPCcontroller.torque[7],MPCcontroller.torque[8]);
        std::cout << "joint 1 : " << RearRightExternalTorqueObserver.GetResidualVector()[0] << " joint 2 : " << RearRightExternalTorqueObserver.GetResidualVector()[1] << " torque1 :" << MPCcontroller.torque[7] << " torque2 :" << MPCcontroller.torque[8] << std::endl;
    }
